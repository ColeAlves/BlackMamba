let mic, fft; //fft = fouir transform
var song;
var slider;
var img, originalImg;
var milliseconds;
...
function draw() {
  ...
  */
  ...
  //adjustAllRGBAFilter(0,size*10,200);

  if(size > 2.5){
    //adjustGreyCircleFilter(0,0,0,0,size);
  }
  adjustGreyHoleFilter(0,0,0,0,size,100);
  //Show edited image
...
function adjustDistFilter(rAdj = 0, gAdj = 0, bAdj = 0, aAdj = 0) {

  img.loadPixels();
  for (let x = 0; x < (img.width); x++) {
    for (let y = 0; y < (img.height); y++) {
      var index = (x + y * img.width) * 4;
      let editedR = img.pixels[index + 0] - dist(x, y, img.width / 2, img.height / 2) + rAdj; //r value
      let editedG = img.pixels[index + 1] - dist(x, y, img.width / 2, img.height / 2) + gAdj; //g value
      let editedB = img.pixels[index + 2] - dist(x, y, img.width / 2, img.height / 2) + bAdj; //b value
      let editedA = img.pixels[index + 3] - dist(x, y, img.width / 2, img.height / 2) + aAdj; //a value
...
function adjustGreyCircleFilter(rAdj = 0, gAdj = 0, bAdj = 0, aAdj = 0, amplitudeNumber = 0) {
 let circleX = random(0, img.width);
  let circleY = random(0, img.height);
  img.loadPixels();
  for (let x = 0; x < (img.width); x++) {
    for (let y = 0; y < (img.height); y++) {
      var index = (x + y * img.width) * 4;
...
function adjustGreyHoleFilter(rAdj = 0, gAdj = 0, bAdj = 0, aAdj = 0, amplitudeNumber = 0, circleDiameter = 25) {
 let circleX = random(0, img.width);
  let circleY = random(0, img.height);
...
function adjustGreyHoleFilter(rAdj = 0, gAdj = 0, bAdj = 0, aAdj = 0, amplitudeNumber = 1, circleDiameter = 25) {
...
function ellipseJumpFilter(rAdj = 0, gAdj = 0, bAdj = 0, aAdj = 0, amplitudeNumber = 0) {
 let circleX = random(0, img.width);
  let circleY = random(0, img.height);
  img.loadPixels();
  for (let x = 0; x < (img.width); x++) {
